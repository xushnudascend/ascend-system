  ...
  return (
    <div className="relative min-h-screen bg-background flex items-center justify-center px-4 py-10 overflow-hidden">
      ...
      <div aria-hidden className="pointer-events-none absolute inset-0 -z-10">
        <div className="absolute -top-32 left-1/2 -translate-x-1/2 h-[480px] w-[680px] rounded-full bg-primary/20 blur-[120px] opacity-60" />
        <div className="absolute bottom-[-160px] right-[-120px] h-[420px] w-[420px] rounded-full bg-accent/15 blur-[120px] opacity-50" />
      </div>
      <SEO
      ...
      >
        <div className="text-center mb-8">
          ...
          <p className="text-muted-foreground mt-2 text-sm">
            {isLogin ? "Tizimga kiring va davom eting" : "Yangi hisob yarating"}
          </p>
        </div>

        <div className="grid grid-cols-2 gap-3 mb-4">
          <button type="button" onClick={() => oauth("google")} disabled={loading} aria-label="Google bilan kirish" className="py-2.5 rounded-xl border border-border bg-card text-sm font-medium flex items-center justify-center gap-2 hover:bg-muted hover:border-primary/40 transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
          ...
          <button type="button" onClick={() => oauth("apple")} disabled={loading} aria-label="Apple bilan kirish" className="py-2.5 rounded-xl border border-border bg-card text-sm font-medium flex items-center justify-center gap-2 hover:bg-muted hover:border-primary/40 transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="p-6 rounded-2xl border border-border bg-card/80 backdrop-blur-sm shadow-xl shadow-black/20 space-y-4">
            {!isLogin && (
              <div>
                <label htmlFor="auth-name" className="text-xs text-muted-foreground">Ism</label>
                <input
                  id="auth-name"
                  autoComplete="name"
                  value={name}
            ...
            <div>
              <label htmlFor="auth-password" className="text-xs text-muted-foreground">Parol</label>
              <div className="relative mt-1">
                <input
                  ...
                  required
                  minLength={6}
                  className="w-full px-4 py-3 rounded-xl border border-border bg-background text-sm focus:outline-none focus:ring-2 focus:ring-primary/50 transition-all pr-11"
                />
                <button
                  type="button"
              ...
              {!isLogin && (
                <p className="mt-1.5 text-[11px] text-muted-foreground">Kamida 6 ta belgi.</p>
              )}
            </div>

            {!isLogin && (
              <label className="flex items-start gap-2 text-xs text-muted-foreground cursor-pointer leading-relaxed">
                <input type="checkbox" checked={agreed} onChange={e => setAgreed(e.target.checked)} className="mt-0.5 accent-primary shrink-0" />
            ...
            <button
              ...
              className="w-full py-3 rounded-xl bg-primary text-primary-foreground font-heading font-bold glow-box hover:brightness-110 transition-all flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:brightness-100"
            >
              {loading ? (
                <><Loader2 className="w-4 h-4 animate-spin" /> Iltimos kuting…</>
              ) : (
                <>
